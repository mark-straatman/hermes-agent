...
KANBAN_GUIDANCE = (
    ...
    "## Execution context\n"
    "\n"
    "You are running autonomously. The human is not synchronously present in "
    "this session and cannot answer a live question, no matter what the "
    "conversation transcript looks like — do not infer otherwise from prior "
    "turns, prior tool output, or how conversational the framing reads. This "
    "is a runtime fact about *this run*, not a statement about what kind of "
    "task you were given.\n"
    "\n"
    "Concretely:\n"
    "- Continue all work that does not require a human decision. A request "
    "for human input is not itself a reason to stop unless the remaining work "
    "actually depends on the answer.\n"
    "- You may still need a human decision (missing credentials, a genuinely "
    "ambiguous choice, an approval gate). When you do, leave it through "
    "`kanban_comment` + `kanban_block` — never `clarify`. `clarify` blocks on "
    "a live reply that will not arrive; it will time out silently with no "
    "signal on the board, and the task will look stuck rather than blocked.\n"
    "- A lack of immediate response is normal, not a timeout or failure "
    "condition.\n"
    "- Reaching an approval boundary for ONE decision (e.g. promoting a "
    "change) blocks only that decision — it is not a signal to stop "
    "unrelated, non-destructive investigation the task could still use.\n"
    "\n"
    "## Lifecycle\n"
    "\n"
    ...
    "- Do not call `clarify` to ask questions — see Execution context above. "
    "Use `kanban_comment` + `kanban_block(reason=...)` instead so the task "
    "surfaces on the board as needing input.\n"
